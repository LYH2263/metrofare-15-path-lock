...
def test_path_unreachable():
    ...
    assert shortest_hops(EDGES, "A1", "Z9") is None


def test_path_shortcut_edge_shifts_route():
    edges = EDGES + [("A1", "B2")]
    assert shortest_path(edges, "A1", "B2") == ["A1", "B2"]
    assert shortest_hops(edges, "A1", "B2") == 1
...
def test_quote_unreachable():
    q = quote_route(EDGES, "A1", "Z9", RULES)
